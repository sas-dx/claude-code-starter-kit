...
各機能の詳細な解説・具体例・サンプルコードをまとめた HTML ドキュメントサイトを公開しています。

**https://sas-dx.github.io/claude-code-starter-kit/**
...
Skills・Agents・Hooks・MCP サーバー等を1つのパッケージにまとめて配布・共有できる仕組みです。`.claude/` の設定がそのプロジェクト専用なのに対し、Plugins はチームや他プロジェクトへの横展開に適しています。

- `.claude/` で試作 → 良かったものを Plugin 化して共有、が推奨フロー
- スキル名は `/plugin-name:skill-name` の名前空間で衝突を防止
- `claude --plugin-dir ./my-plugin` でローカルテスト可能

> 詳細: [.claude/plugins/README.md](.claude/plugins/README.md)
...
ツールに詳しい1人のエンジニアが全てを整備するのではなく、**チーム全員が少しずつ改善を積み重ねる**ことで、プロジェクト全体の開発体験が向上していきます。このリポジトリの CLAUDE.md、Rules、Skills、Agents、Commands は全て、そのための土台です。
